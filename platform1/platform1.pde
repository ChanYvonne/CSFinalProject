...
void checkForPlayerBumping(){
  ...
  //left side low
  if(chrisleftSideLow.x <= thomasrightSideLow.x && chrisleftSideLow.y >= thomastopSide.y && chrisrightSideLow.x >= thomasleftSideLow.x && chrisleftSideHigh <= theThomas.getPosY()) {
 ...
 // left side high
  if(chrisleftSideLow.x <= thomasrightSideLow.x && chrisleftSideHigh.y <= theThomas.getPosY() && ) {
    theChris.setPosX(thomasrightSideHigh.x+chriswallProbeDistance);
    if(theChris.getVelocityX() < 0) {
      theChris.setPosX(theChris.getPosX() - theChris.getVelocityX());
      theChris.setPosY(theChris.getPosY() - theChris.getVelocityY());
